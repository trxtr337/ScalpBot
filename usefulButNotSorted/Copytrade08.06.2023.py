...
def main_program():
   global User, user_id, Username, API_KEY, SECRET_KEY, Trading_volume,Сomissoin_percentage
   ...
   Сomissoin_percentage=filtered_rows[GG][8]
   print(f"User ID: {user_id}\n"
         f"Username: {Username}\n"
         f"API_KEY: {API_KEY}\n"
         f"SECRET_KEY: {SECRET_KEY}\n"
         f"Trading Volume: {Trading_volume}\n"
         f"Commission Percentage: {Сomissoin_percentage}\n")
...
def binance_post_request(endpoint, params=None):
   ...
   response = requests.post(url, headers={'X-MBX-APIKEY': API_KEY}, data=params)
...
async def create_market_order(symbol, side, price, Y):
   global quantity_list, price_list, Trading_volume, data_to_write,Сomissoin_percentage
   ...
   if side_list[-1] != side:
       ...
       if side == 'SELL':
           ...
           trade = volume * change
           if trade > 0:
               comission = trade * float(Сomissoin_percentage)


               data_to_write.append([user_id, Username, None, None, comission, None, None])
