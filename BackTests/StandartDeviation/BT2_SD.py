...
def calculate_bollinger_bands(df, window, std_multiplier):
    df['mean'] = df['close'].rolling(window=window).mean()
    df['std'] = df['close'].rolling(window=window).std()
    df['upper_band'] = df['mean'] + std_multiplier * df['std']
    df['lower_band'] = df['mean'] - std_multiplier * df['std']
    df['mid_band_upper'] = df['mean'] + (std_multiplier / 2) * df['std']
    df['mid_band_lower'] = df['mean'] - (std_multiplier / 2) * df['std']
    return df

# Торговая стратегия
def trading_strategy(df, window, std_multiplier, stop_loss):
    df = calculate_bollinger_bands(df, window, std_multiplier)
    df['signal'] = 0
    position = 0
    entry_price = 0
    in_ban = False
...
def main():
    ...
    std_multiplier = st.sidebar.slider("Множитель стандартного отклонения", min_value=1.0, max_value=5.0, value=3.0, step=0.1)
    ...
    if st.sidebar.button("Запустить бектест"):
        ...
        df = trading_strategy(df, window, std_multiplier, stop_loss)
