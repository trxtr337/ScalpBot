...
# Торговая стратегия с противоположной логикой
        ...
        if not in_ban:
            if position == 0:
                if df['close'].iloc[i] < df['lower_band'].iloc[i]:
                    df.at[i, 'signal'] = -1  # Short signal (opposite of buy)
                    position = -1
                    entry_price = df['close'].iloc[i]
                elif df['close'].iloc[i] > df['upper_band'].iloc[i]:
                    df.at[i, 'signal'] = 1  # Buy signal (opposite of short)
                    position = 1
                    entry_price = df['close'].iloc[i]
            elif position == 1:
...
def main():
    ...
    symbol = st.sidebar.selectbox("Выберите монету", ['BTC/USDT', 'ETH/USDT', 'BNB/USDT', 'XRP/USDT', 'DOGE/USDT'])
    timeframe = st.sidebar.selectbox("Выберите временной интервал", ['1m', '5m', '15m', '1h', '1d'])

    # Выбор дат
    start_date = st.sidebar.date_input("Дата начала", pd.to_datetime("2018-01-01"))
    end_date = st.sidebar.date_input("Дата окончания", pd.to_datetime("2024-01-31"))
